...
export default function AdminPasswordChangePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    setError("");
    try {
      await adminRequest("/admin/change-password", { method: "POST", body: JSON.stringify(form) });
      navigate("/admin");
    } catch (err) {
        ...
        <div className="flex flex-col justify-center px-5 py-12 sm:px-8 sm:py-16 md:px-12 lg:px-16 xl:px-20">
          <div className="mx-auto w-full max-w-sm">
            ...
            <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
              {loading ? "Updating your password..." : "Enter your current password and choose a new one."}
            </p>
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4 sm:mt-8">
                <div>
                  <label htmlFor="currentPassword" className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                    Current password
                  </label>
                  <input
                    id="currentPassword"
