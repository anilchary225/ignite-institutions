...
function RankerCard({ ranker, rank: position }) {
  const c = badgeColors[ranker.badge] || badgeColors.orange;
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col justify-between items-center rounded-3xl bg-white px-6 py-8 text-center shadow-sm ring-1 ring-neutral-100 transition hover:shadow-xl dark:bg-neutral-900 dark:ring-neutral-800"
    >
      {/* position badge */}
      {position <= 3 && (
        <motion.div
      ...
      <div className={`relative mt-2 h-42 w-32 overflow-hidden rounded-xl ring-4 ${c.ring}`}>
        {ranker.avatar ? (
          ...
          <span className={`grid h-full w-full place-items-center bg-linear-to-br text-4xl font-black text-white ${c.nameBg}`}>
            {ranker.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join("")}
          </span>
        )}
      </div>
...
export default function JCRankers() {
  return (
    <section id="rankers" className="bg-neutral-50 px-6 py-20 dark:bg-neutral-900/40 overflow-hidden">
      <div className="mx-auto max-w-7xl">
        ...
        >
          {rankers.map((ranker, i) => (
            <RankerCard key={ranker.hallTicket || ranker.name} ranker={ranker} rank={i + 1} />
          ))}
        </motion.div>
      </div>
    </section>
