...
function InputField({ label, name, value, onChange, type = "text", required = false, icon: Icon }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={name} className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 uppercase tracking-wide">
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={onChange}
          required={required}
          className={`w-full ${Icon ? "pl-9" : "pl-3"} pr-3 py-2.5 rounded-lg border border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white text-sm placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all`}
        />
      </div>
    </div>
  );
...
const contactInfo = [
  { icon: Phone,  label: "Call Us",      value: "+91 98765 43210",      color: "bg-green-700" },
  { icon: Mail,   label: "Email",        value: "admissions@ignitejc.com", color: "bg-blue-500" },
  { icon: MapPin, label: "Campus",       value: "Hyderabad, Telangana", color: "bg-orange-500" },
  { icon: Clock,  label: "Office Hours", value: "Mon – Sat, 9am – 6pm", color: "bg-red-500" },
];

export default function Form() {
  const [formData, setFormData] = useState({ firstName: "", lastName: "", email: "", phone: "", interests: [], message: "" });
        ...
        <div className="grid lg:grid-cols-[320px_1fr] gap-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-neutral-50 dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 p-5 shadow-sm">
              <h3 className="text-sm font-bold text-neutral-900 dark:text-white mb-4">Contact Information</h3>
              <div className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl bg-orange-500 p-5 text-white">
              ...
              <p className="text-xs text-white/90 leading-relaxed">
                MPC (IIT-JEE) and BiPC (NEET) seats are filling fast. Submit your enquiry and our team will call you within 24 hours.
              </p>
          ...
          <div className="bg-neutral-50 dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 p-5 sm:p-7 shadow-sm">
            {submitted ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 py-12 text-center">
                <div className="w-14 h-14 rounded-full bg-green-100 dark:bg-green-700/20 flex items-center justify-center text-green-600 dark:text-green-400 text-2xl font-black">✓</div>
                <h3 className="text-lg font-bold text-neutral-900 dark:text-white">Message Sent!</h3>
                <p className="text-sm text-neutral-500 dark:text-neutral-400 max-w-xs">Our admissions team will contact you within 24 hours.</p>
                <button onClick={() => setSubmitted(false)} className="text-sm text-orange-500 font-semibold hover:underline">Send another</button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <InputField label="Phone" name="phone" type="tel" value={formData.phone} onChange={handleChange} icon={Phone} />
                </div>
                <div>
                  <p className="text-xs font-bold text-neutral-600 dark:text-neutral-400 uppercase tracking-wide mb-2">MPC — Intermediate</p>
                  <div className="flex flex-wrap gap-2">
                    {mpcOptions.map((opt) => <Chip key={opt} label={opt} active={formData.interests.includes(opt)} onClick={() => toggleInterest(opt)} />)}
                  </div>
                </div>
                <div>
                  <p className="text-xs font-bold text-neutral-600 dark:text-neutral-400 uppercase tracking-wide mb-2">BiPC — Intermediate</p>
                  <div className="flex flex-wrap gap-2">
                    {bipcOptions.map((opt) => <Chip key={`bipc-${opt}`} label={opt} active={formData.interests.includes(`BiPC-${opt}`)} onClick={() => toggleInterest(`BiPC-${opt}`)} />)}
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="message" className="text-xs font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-wide">Message</label>
                  <div className="relative">
                    <MessageSquare size={15} className="absolute left-3 top-3 text-neutral-400" />
                    <textarea
                ...
                <button
                  type="submit"
                  className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 transition-colors text-white font-semibold px-6 py-2.5 rounded-xl text-sm shadow-sm shadow-orange-500/20"
                >
