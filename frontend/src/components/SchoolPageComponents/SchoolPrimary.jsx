...
function GradeCard({ grade: g }) {
  const c = colorMap[g.color] || colorMap.sky;
  return (
    <motion.div
      variants={cardReveal}
      whileHover={{ y: -6, transition: { duration: 0.25 } }}
      className={`flex flex-col gap-4 rounded-3xl border p-6 ${c.bg} ${c.border}`}
    >
      <div className="flex items-center gap-3">
        <motion.div
          whileHover={{ scale: 1.15, rotate: 6 }}
          className={`flex h-12 w-12 items-center justify-center rounded-2xl text-2xl ${c.emoji}`}
        >
          {g.emoji}
        </motion.div>
        <div>
...
export default function SchoolPrimary() {
  return (
    <section id="primary" className="bg-white px-6 py-20 dark:bg-neutral-950 overflow-hidden">
      <div className="mx-auto max-w-7xl">

        {/* ── Classes 6-10 section label ── */}
        <motion.div
          initial="hidden"
          whileInView="visible"
        ...
        <motion.div
          ...
          className="mt-10 overflow-hidden rounded-3xl bg-linear-to-br from-amber-400 to-orange-500 p-8 shadow-lg"
        >
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div
                key={time}
                className="flex items-center gap-3 rounded-2xl bg-white/20 px-4 py-3 backdrop-blur-xs"
              >
