      ...
      <div className="px-4 sm:px-8">
        ...
        <section className="py-14 sm:py-16">
          <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2 md:items-center">
            <div>
              <span data-aos="fade-up" className="inline-block text-xs font-bold tracking-[0.25em] text-orange-500 uppercase mb-3">
                Looking Forward
              </span>
              <h2 data-aos="fade-up" className="text-2xl font-extrabold text-neutral-900 dark:text-white sm:text-3xl">
                {vision.heading}
              </h2>
              <div data-aos="fade-up" className="mt-2 h-0.5 w-10 bg-orange-500 rounded" />
              <div className="mt-4 space-y-3 text-neutral-600 dark:text-neutral-300 text-sm leading-relaxed">
                {vision.paragraphs.map((paragraph, index) => (
                  <p data-aos="fade-up" key={index}>{paragraph}</p>
                ))}
              </div>
            </div>

            <div data-aos="fade-up" className="overflow-hidden  p-6 flex items-center justify-center">
              <img
                src={visionImage}
        ...
        <section className="py-14 sm:py-16 border-t border-neutral-100 dark:border-neutral-800">
          <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2 md:items-center">
            <div data-aos="fade-up" className="overflow-hidden rounded-2x p-6 flex items-center justify-center order-2 md:order-1">
              <img
                src={missionImage}
            ...
            <div className="order-1 md:order-2">
              <span data-aos="fade-up" className="inline-block text-xs font-bold tracking-[0.25em] text-blue-500 uppercase mb-3">
                Our Purpose
              </span>
              <h2 data-aos="fade-up" className="text-2xl font-extrabold text-neutral-900 dark:text-white sm:text-3xl">
                {mission.heading}
              </h2>
              <div data-aos="fade-up" className="mt-2 h-0.5 w-10 bg-blue-500 rounded" />
              <div className="mt-5 space-y-5">
                {mission.items.map((item, i) => (
                  <div data-aos="zoom-in" key={item.title} className="flex gap-4">
                    <span data-aos="zoom-in" className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-white text-xs font-bold mt-0.5 ${i === 0 ? "bg-blue-500" : "bg-green-700"}`}>
                      {i + 1}
                    </span>
                    <div>
