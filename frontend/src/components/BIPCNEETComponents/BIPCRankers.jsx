...
function RankerCard({ r, pos }) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col items-center rounded-3xl bg-white px-6 py-8 text-center shadow-sm ring-1 ring-neutral-100 transition hover:shadow-xl dark:bg-neutral-900 dark:ring-neutral-800"
    >
      {pos <= 3 && (
        <motion.div
          initial={{ scale: 0 }}
          ...
          className={`absolute -top-3 left-1/2 -translate-x-1/2 flex h-7 w-7 items-center justify-center rounded-full text-xs font-black text-white shadow-md ${c.num}`}
        >
          {pos}
        </motion.div>
      )}
      <div className={`relative mt-2 h-42 w-32 overflow-hidden rounded-2xl ring-4 ${c.ring}`}>
        {r.avatar ? (
          ...
          <span className={`grid h-full w-full place-items-center bg-linear-to-br text-4xl font-black text-white ${c.grad}`}>
            {r.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join("")}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 w-full rounded-2xl bg-linear-to-br px-4 py-4">
        ...
        <p className="text-xs text-black/70 dark:text-white/70 mt-0.5">{r.rank}</p>
      </div>
